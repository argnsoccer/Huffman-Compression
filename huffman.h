...
LeafNode::LeafNode(char character, int freq)
{
    data = character;
    frequency = freq;
    isLeaf = true;
...
BinTree::~BinTree()
{
    ...
    for(int j = 0; j < min2Freqs.size(); j++)
    {
        delete min2Freqs[j];
    }
}
...
void Node::createBitStream()
{
    ...
    {
        children[i]->bitstream = bitstream;//getting it initialized with the bitstream 
